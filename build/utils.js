...
const updateAllPlayersReadyStatus = () => {
    for (const gameId in aw.games) {
        ...
        GAME.playersReady = GAME.playerIds.every((playerId) => aw.players[playerId].readyStatus);
    }
};
const updateAllPlayersAnsweredStatus = () => {
    for (const gameId in aw.games) {
        ...
        GAME.playersAnswered = GAME.playerIds.every((playerId) => aw.players[playerId].answeredStatus);
    }
};
const getAnswers = (gameId) => {
    ...
    for (const playerId of aw.games[gameId].playerIds) {
        const PLAYERANSWER = { answer: aw.players[playerId].answers[QUESTIONINDEX].answer, answerType: '' }; //aw.players[playerId].answers[QUESTIONINDEX].answer, answerType: '' };
...
export { generateId, updateAllPlayersReadyStatus, getCurrentTime, appendStatsToLog as drawDebug, boolConv, newPlayerObj, newGameObj, updateAllPlayersAnsweredStatus, processAnswers, getAnswers, getMiddleIndex, getHighestOdds, isHost, CSVToJSON, getUniqueID, debug, writeLog, appendStatsToLog, getPlayerList };
